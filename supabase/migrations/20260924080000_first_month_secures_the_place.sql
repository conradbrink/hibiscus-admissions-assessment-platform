...
--
-- Each schedule also gains a `tuition_annual` line, three terms to the year,
-- so the letter can show the parent how the monthly figure was arrived at
-- instead of asking them to take it on trust.
--
-- The monthly figure is the year over nine, which is the term over three —
-- the same number by two routes, because the invitation's own plan divides
-- the same three terms into nine instalments. Where the division is not exact
-- it rounds to the thebe: Stage 1's P 51,000 a year becomes P 5,666.67 a
-- month, which is not a number anybody would choose, and the school may want
-- a rounder one. It is left as the arithmetic rather than rounded by me,
-- because a figure invented here is one nobody can trace back.
--
...
-- ---------------------------------------------------------------------------
-- The yearly line, so the monthly one can be shown to add up
-- ---------------------------------------------------------------------------

-- A parent reading "P 4,130.00 a month" has to take it on trust. A parent
-- reading "P 37,170.00 for the year, in nine instalments of P 4,130.00" can
-- check it, and does not have to multiply a term by three in their head to
-- find out what the award is actually worth.
--
-- Three terms to a year, and the invitation's own instalment plan divides that
-- same year into nine, so the monthly figure is the year over nine and the
-- term over three at once: 53,100 / 9 = 5,900 = 17,700 / 3. The two lines
-- cannot drift, because both are computed from the same term.
--
-- Not payable at acceptance: it is what the year costs, not a bill. Only the
-- monthly line is asked for up front.

insert into public.fee_lines (schedule_id, code, label, amount_minor, payable_at_acceptance, position)
select fs.id,
       'tuition_annual',
       'Tuition per year',
       (term.amount_minor * 3)::bigint,
       false,
 ...
 where ay.label = '2027'
   and fs.status = 'active'
   and not exists (
     select 1 from public.fee_lines x where x.schedule_id = fs.id and x.code = 'tuition_annual'
   );
...
select p.id, e.position, 'discount_percent', e.fee_code,
       (regexp_replace(p.code, '\D', '', 'g'))::numeric,
       'Scholarship — ' || regexp_replace(p.code, '\D', '', 'g') || '% of ' || e.what
  from public.promotions p
  cross join (values
    (35, 'tuition_annual', 'the year''s tuition'),
    (40, 'tuition_month', 'the monthly fee')
  ) as e(position, fee_code, what)
 where p.code like 'SCHOLARSHIP-%'
   and not exists (
     select 1 from public.promotion_effects x
      where x.promotion_id = p.id and x.fee_code = e.fee_code
   );
...
--
-- The table shows the YEAR and the month, not the term. A parent asked for
-- P 4,130.00 a month can only check it against something, and "P 37,170.00 for
-- the year, in nine instalments" is that something; a termly figure would have
-- them dividing by three and multiplying by nine to get there. The nine months
-- are named because the school already named them: the invitation says "nine
-- equal instalments, the first due before 31 December 2026 and the last in
-- August 2027", so the bracket repeats the school's own plan rather than
-- inventing a schedule. That makes the dates specific to the 2027 intake, as
-- the rest of this template already is.
--
-- The award is named by `{{promotion_name}}`, which reads "Scholarship — 30%".
-- That is a label, not a noun phrase, so it is introduced as one ("Neo's
-- award: Scholarship — 30%.") rather than dropped into a sentence, where it
-- stutters: "Neo holds a Scholarship — 30% scholarship."
--
...
begin
  ...
  -- And a yearly line to show what the nine instalments add up to.
  select string_agg(fs.name, ', ' order by fs.name) into v_missing
    from public.fee_schedules fs
    join public.academic_years ay on ay.id = fs.academic_year_id
    join public.fee_lines term on term.schedule_id = fs.id and term.code = 'tuition_term'
   where ay.label = '2027' and fs.status = 'active'
     and not exists (
       select 1 from public.fee_lines a where a.schedule_id = fs.id and a.code = 'tuition_annual'
     );
  if v_missing is not null then
    raise exception 'no yearly line on: %', v_missing;
  end if;

  -- The two must agree, or the letter shows a year that is not nine of the
  -- month it sits above. Both are computed from the same term, so this can
  -- only fail if somebody edits one by hand — which is exactly when a parent
  -- would be the one to notice.
  select string_agg(fs.name || ' (' || a.amount_minor || ' <> 9 x ' || m.amount_minor || ')', ', ' order by fs.name)
    into v_mismatch
    from public.fee_schedules fs
    join public.academic_years ay on ay.id = fs.academic_year_id
    join public.fee_lines a on a.schedule_id = fs.id and a.code = 'tuition_annual'
    join public.fee_lines m on m.schedule_id = fs.id and m.code = 'tuition_month'
   where ay.label = '2027' and fs.status = 'active'
     and abs(a.amount_minor - m.amount_minor * 9) > 9;
  if v_mismatch is not null then
    raise exception 'the yearly figure is not nine monthly instalments on: %', v_mismatch;
  end if;

  -- Every scholarship band discounts both, or a family reads an undiscounted
  -- figure beside a discounted one and cannot tell which is the deal.
  select string_agg(p.code || ' (' || want.fee_code || ')', ', ' order by p.code, want.fee_code) into v_undiscounted
    from public.promotions p
    cross join (values ('tuition_month'), ('tuition_annual')) as want(fee_code)
   where p.code like 'SCHOLARSHIP-%' and p.is_active
     and not exists (
       select 1 from public.promotion_effects e
        where e.promotion_id = p.id and e.fee_code = want.fee_code and e.kind = 'discount_percent'
     );
  if v_undiscounted is not null then
    raise exception 'these bands would show an undiscounted figure: %', v_undiscounted;
  end if;
  ...
  raise notice 'every 2027 tuition schedule quotes a year and nine instalments, the first payable at acceptance, every scholarship band discounts both, and every active offer template renders';
end $$;
