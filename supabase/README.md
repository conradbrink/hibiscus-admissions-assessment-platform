...
| `…20260914110000_enquiry_form_never_edits_a_stranger` (14 Sep) | `create_application(..., p_trusted boolean default false)`: an anonymous enquiry naming an address already on file no longer rewrites that contact's name and number, nor the child's names on the matching application — it only adds a new child; the form passes `p_trusted` when the browser already holds that family's session, and the walk-in form always does. `book_session()` refuses a session at another campus (`session_wrong_campus`). Security suite cases 66 and 67 |
...
| `…20260919100000_crm_core` (19 Sep) | The CRM's foundation: seven `crm.*` permissions and three roles (marketing, reception, teacher) with their grants; `families` and `contacts` gain the CRM columns (campus, lifecycle stage, lead source, tags, assignee, follow-up, consent per channel with when and by whom, unsubscribe token); `can_access_family()`/`can_edit_family()` and rewritten family and contact policies; `crm_create_family()` for a walk-in typed in at the desk; `crm_trigger_events` (the outbox, service role only) written by triggers on applications, students, re-enrolment, contacts and both message logs; `crm_compute_lifecycle()`/`crm_sync_family()`; `crm_notes`; `audit_log.family_id`; a backfill of every existing family; the CRM settings |
| `…20260919100100_crm_opportunities` (19 Sep) | `opportunity_types` (seeded from the school's list), `opportunity_rules` (seven, all off), `opportunities` with one open row per family, type and child; the currency and default value copied on the way in; `crm_opportunity_summary()` |
| `…20260919100200_crm_segments_campaigns` (19 Sep) | `v_crm_family_facts` (security invoker: one row per family with everything a list, a rule or a count needs), `segments`, `campaigns`, `campaign_recipients`; `campaigns_guard_transition()` (a second pair of eyes, a third for fee, policy and group-wide notices, wording locked once approved, "sending" and "sent" the engine's alone); `crm_campaign_stats()` |
