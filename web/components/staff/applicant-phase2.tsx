...
export async function ApplicantPhase2({
  ...
  const [{ data: scores }, { data: messages }, { data: messageTemplates }] = await Promise.all([
    ...
    supabase.from("message_templates").select("key, name, is_active, meta_template_name, twilio_content_sid").eq("is_active", true).order("name"),
  ]);
      ...
      <Tabs defaultValue={app.requires_assessment ? "assessment" : "decision"}>
        ...
        <TabsContent value="messages" className="text-sm">
          <MessagesPanel
            ...
            templates={(messageTemplates ?? []).filter((t) => t.meta_template_name || t.twilio_content_sid)}
